...
public class PrefixContainer implements Comparable<PrefixContainer>
{
    ...
    private List<RouteHandler> routes = new LinkedList<RouteHandler>();
     ...
     */
    public RouteHandler getHandler(BalsaRequest request)
    {
        for (RouteHandler route : this.routes)
        {
    ...
    public RouteHandler getExceptionHandler(BalsaRequest request, Throwable t)
    {
        for (RouteHandler route : this.routes)
        {
