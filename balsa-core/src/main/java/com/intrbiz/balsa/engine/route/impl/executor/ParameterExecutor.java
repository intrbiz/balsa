...
public class ParameterExecutor implements RouteExecutorFactory
{
    @Override
    public boolean match(final Method handler, final String[] as, final Router router)
    {
        return void.class.equals(handler.getReturnType()) && handler.getParameterTypes().length == as.length && isParameters(handler.getParameterTypes());
    }
